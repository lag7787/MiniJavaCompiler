...
public abstract class SimpleMathInstruction extends Instruction {
	...
	// rm,r variants
	public SimpleMathInstruction(R modrmsib) { // i guess this means filling the modrmsib
		byte[] modrmsibBytes = modrmsib.getBytes(); // gets the modrmsib portion of the instruction
		importREX(modrmsib);
	...
	public SimpleMathInstruction(R modrmsib, int imm) {
		...
		if( x64.isOneByte(imm) ) {
			// opcode for simple math immediates of only one bye
			opcodeBytes.write(0x83);
			x64.writeBytes(immBytes,modrmsibBytes);
			immBytes.write(imm);
		} else {
			// opcode for simple math immediates of more than one byte
			opcodeBytes.write(0x81);
			x64.writeBytes(immBytes,modrmsibBytes); //write the modrsmib which gives the 
